...
B、C、D互相为兄弟节点，B的高度是2，B的深度是1，树的高度是3
...
二叉树中，每一个节点的左右子树的高度差不能大于1，称为平衡二叉树
...
- **完全二叉树**：深度为h，除了第h层外，其他各层（1～h-1）的节点数都大道最大个数，第h层所有的节点都连续集中在最左边
...
如果我们选取节点B`i=2`，则它的父节点为`i / 2 = 1`，左子节点`2i = 4`，右子节点`2i + 1 = 5`
...
因此我们可以把完全二叉树存储在数组里（从下标为1开始存储），完全可以通过下标找到任意节点的父子节点，从而构建出完全二叉树。
